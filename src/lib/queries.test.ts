...
test('закрытая лавка уходит из списков, но не из прошлых дней', async () => {
  ...
  const before = await q.listShops('2026-08-25');
  const after = await q.listShops('2026-09-14');

  assert.ok(
    before.some((s) => s.code === 'М15'),
    'за август М15 обязана быть: её дни остаются в радаре',
  );
  assert.ok(
    !after.some((s) => s.code === 'М15'),
    'с 14.09 М15 не должна попадать в списки и счётчики',
  );
  assert.equal(after.length, before.length - 1, 'закрытие убрало не только М15');

  // Без даты справочник прежний: там, где период ни при чём, лавка остаётся.
  assert.ok((await q.listShops()).some((s) => s.code === 'М15'));

  // История лавки открывается как прежде — карточка закрытой лавки рабочая.
