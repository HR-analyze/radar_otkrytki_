...
export interface ContestScore {
  ...
  red: number;
  /**
   * Сколько оценок сложилось в этот балл — знаменатель для среднего.
   *
   * У одной лавки это её дни с заполненной витриной. Но счёт складывают
   * (sumScores): у РМ и у сети то же поле — уже **лавко-дни**, одиннадцать
   * лавок за два дня дают 22. Подписывать такую сумму «днями» нельзя —
...
/**
 * Средний балл одной лавки за один день: сумма ÷ оценённые лавко-дни.
 *
 * Регионы сравнивать по сумме нельзя: у РМ с двенадцатью лавками она больше
 * просто потому, что лавок больше. null — оценок нет вовсе.
 */
...
/**
 * Первый день конкурса — 14.09.2026.
 *
 * Конкурс объявили в середине месяца, и дни до него лавкам не засчитываются:
 * на вкладке конкурса они не участвуют ни в баллах, ни в календаре. Радар
 * при этом показывает историю целиком — там она про другое.
 */
export const CONTEST_START = '2026-09-14';
