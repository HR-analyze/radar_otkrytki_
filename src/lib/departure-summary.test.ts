...
test('на карточке лавки: «выгрузки нет» и «отметок нет» — разные вещи', async () => {
  writeSnapshot(
    // Выгрузка по РЦ есть только за 19-е.
    [row('2026-09-19', 'Ороспаев Павел Юрьевич', '04:00', '04:20')],
    [
      mark('2026-09-19', 'Соловьев Михаил Михайлович', 'М1', '06:05'),
      mark('2026-09-20', 'Соловьев Михаил Михайлович', 'М1', '06:05'),
    ],
  );

  const { shopHistory } = await import('./queries');
  const days = await shopHistory('М1', '2026-09-19', '2026-09-20');
  const byDate = new Map(days.map((d) => [d.date, d]));

  assert.deepEqual(
    byDate.get('2026-09-19')!.departureUnits,
    ['РЦ Свобода'],
    'за 19-е выгрузка есть — значит «человек не отметился» сказать можно',
  );
  assert.deepEqual(
    byDate.get('2026-09-20')!.departureUnits,
    [],
    'за 20-е выгрузки нет — про отметки мы ничего не знаем',
  );
  // Сам Соловьев не выезжал ни в один из дней: отличает их только выгрузка.
  for (const d of days) assert.deepEqual(d.people[0].departures, []);
});
